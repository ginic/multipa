...
#SBATCH -o %j_combined_train_duration_100_samples.out
...
for i in "${!seeds[@]}"; do
    seed=${seeds[$i]}
    model_dir=data/models/train_duration_100_samples_$i

    echo "Starting training for train_duration_100_samples_$i with seed=$seed"
    ...
    echo "Finished training for train_duration_100_samples_$i"
done
